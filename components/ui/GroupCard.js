...
export default function GroupCard(props) {
  ...
  async function deleteGroup() {
    const responseGroupDelete = await fetch("/api/group/" + groupId, {
      method: "DELETE",
    });
    ...
    const responseExpensesRemove = await fetch(
      "/api/expense/groupall/" + groupId,
      { method: "DELETE" }
    );
  ...
  async function deleteMember(memberEmail) {
    ...
    try {
      ...
      const url =
        "api/expense/membergroupall/" + copyMemberEmail + "&" + groupId;
    ...
    <Fragment>
      ...
      >
        ...
        {props.owner === props.user && (
          <button className={classes.addMember} onClick={handleSearchForm}>
            Add Member
          </button>
        )}
        {searchFormDisplay && (
        ...
        <div className={classes.btnContainer}>
          <Link href={"/group/" + props.id}>
            <button className={classes.viewBtn}>View Group</button>
          </Link>
          {props.owner === props.user && (
            <button className={classes.deleteBtn} onClick={handleDeleteModal}>
              Delete Group
            </button>
          )}
          {props.owner === props.user && (
            <button className={classes.editBtn} onClick={handleEditForm}>
              Edit Name
            </button>
          )}
          {props.owner !== props.user && (
            <button
              ...
              onClick={() => deleteMember(props.email)}
            >
              Leave Group
            </button>
          )}
        ...
        {deleteGroupDisplay && (
          <ConfirmDelete
            message={
              "Are you sure you would like to DELETE this group?, All expenses related to this group will also be deleted"
            }
